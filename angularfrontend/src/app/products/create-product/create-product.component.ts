...
//     if(this.isCreateMode)  {
//       this.productService.createProduct(1 , categoryId , formData)
...
export class CreateProductComponent implements OnInit {
  ...
  onGetProductData() {
    this.productService.getProduct(this.id , this.categoryId).pipe(
      map((productData: Product) => {
        this.image = productData.image;
        this.productForm.patchValue(productData);
      })
...
//    let promotionId: number = this.productForm.controls.promotion.value;
    ...
    if(this.isCreateMode)  {
      this.productService.createProduct( categoryId , formData)
      .subscribe(formProductData => {
        console.log(formProductData);
        this.redirectToProductList();
      });
    ...
    if(!this.isCreateMode) {
      this.productService.updateProduct(this.categoryId, this.id , formData)
        .subscribe(formProductData => {
          console.log(formProductData);
          this.redirectToProductList();
        });
